...
notIndexes <- (cancer$status == 'not')
pcrIndexes <- (cancer$status == 'pcr')
...
xlim <- c(-15,5)
ylim <- c(-8, 6)
plot(proj2D[notIndexes,1], proj2D[notIndexes,2], xlim=xlim, ylim=ylim, xlab='PCA1', ylab='PCA2', col='red')
par(new=T)
plot(proj2D[pcrIndexes,1], proj2D[pcrIndexes,2], xlim=xlim, ylim=ylim, xlab='PCA1', ylab='PCA2', col='blue')
